...
This is modified version for Cloudflare.
...
def is_accepted_request(inc_get_counter) -> bool:
    ...
    if request.path == '/search':
        c_burst = inc_get_counter(interval=20, keys=[b'IP limit, burst', cf_connecting_ip])
        c_10min = inc_get_counter(interval=600, keys=[b'IP limit, 10 minutes', cf_connecting_ip])
        ...
        if 'gzip' not in accept_encoding_list:
            return False
        ...
        if request.args.get('format', 'html') != 'html':
            c = inc_get_counter(interval=3600, keys=[b'API limit', cf_connecting_ip])
            if c > 4:
                return False
    return True
